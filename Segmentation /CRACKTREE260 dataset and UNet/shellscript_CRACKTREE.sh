...
for epoch in "${epochs[@]}"
do
      srun --nodes=1 --ntasks=2 --cpus-per-task=$SLURM_CPUS_PER_TASK python UNet_thesis_CRACKTREE.py "$epoch" > output_epoch_${epoch}.txt & # launch each inner loop iteration as a separate task in the background
done
